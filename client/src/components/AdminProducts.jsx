...
export default function AdminProducts({ products, categories, onRefresh }) {
  ...
  const setSpec = (index, value) =>
    setProductForm({ ...productForm, specs: productForm.specs.map((spec, i) => (i === index ? value : spec)) })
  const addSpec = () => setProductForm({ ...productForm, specs: [...productForm.specs, ''] })
  const removeSpec = (index) =>
    setProductForm({ ...productForm, specs: productForm.specs.filter((_, i) => i !== index) })
  ...
  const uploadInlineImage = async (file) => {
    try {
      const result = await uploadProductImage(file)
      notify.success('Đã chèn ảnh vào nội dung.')
  ...
  const uploadGalleryImages = async (files) => {
    ...
    try {
      ...
      setProductForm((current) => ({
        ...
        gallery: [...(current.gallery || []), ...uploadedImages],
      }))
  ...
  const saveProduct = async (event) => {
    ...
    const payload = {
      ...productForm,
      image: productForm.image || gallery[0] || '',
      gallery,
  ...
  return (
    <div className="admin-crud">
      <form className="quote-form" onSubmit={saveProduct}>
        <h3>
          <Plus /> {productForm.id ? 'Sửa sản phẩm' : 'Thêm sản phẩm'}
        </h3>
        <input
          required
            ...
            {productForm.gallery.map((image) => (
              <div className="gallery-item" key={image}>
                ...
                <button
                  ...
                  className="icon-btn"
                  onClick={() =>
                    setProductForm({ ...productForm, gallery: productForm.gallery.filter((item) => item !== image) })
                  }
                >
                  <Trash2 size={16} />
                </button>
        ...
        />
        <section className="product-content-editor" aria-labelledby="product-content-title">
          <header className="product-content-editor-header">
            <div>
              <span className="product-content-editor-kicker">Trình soạn thảo nâng cao</span>
              <strong id="product-content-title">Nội dung chi tiết sản phẩm</strong>
              <small>Định dạng nội dung, chèn ảnh và bảng thông số trực tiếp.</small>
            </div>
            <span className="product-content-editor-tip">Thanh công cụ luôn hiển thị</span>
          </header>
          <RichTextEditor
            value={productForm.content}
            onChange={(content) => setProductForm((prev) => ({ ...prev, content }))}
        ...
        <div className="spec-editor">
          ...
          {productForm.specs.map((spec, index) => (
            <div className="spec-row" key={index}>
              <input
                placeholder="VD: Tải trọng: 2.000 kg"
                value={spec}
                onChange={(e) => setSpec(index, e.target.value)}
              />
              <button type="button" className="icon-btn" onClick={() => removeSpec(index)}>
                <Trash2 size={16} />
              </button>
            </div>
          ))}
          <button type="button" className="secondary-btn" onClick={addSpec}>
            <Plus size={16} /> Thêm thông số
          </button>
        </div>
        <button className="primary-btn">
          <Save size={16} /> {productForm.id ? 'Cập nhật sản phẩm' : 'Lưu sản phẩm'}
        </button>
        {productForm.id && (
          <button type="button" className="secondary-btn" onClick={() => setProductForm(emptyProduct)}>
            Hủy sửa
          </button>
        )}
      </form>
        ...
        {products.map((p) => (
          <div className="lead-row" key={p.id}>
            ...
            <div className="row-actions">
              <button className="icon-btn" onClick={() => editProduct(p)}>
                <Edit3 size={16} />
              </button>
