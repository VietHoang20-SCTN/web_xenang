...
export default function AdminServices({ services, onRefresh }) {
  ...
  const editService = (service) => setServiceForm({
    ...
    description: service.description || '', icon: service.icon || 'Settings',
    sortOrder: service.sortOrder || 0, isActive: service.isActive ?? true
  })
    ...
    <div className="admin-crud">
      <form className="quote-form" onSubmit={saveService}>
        ...
        <select value={serviceForm.icon} onChange={(e) => setServiceForm({ ...serviceForm, icon: e.target.value })}>
          {Object.keys(serviceIcons).map((icon) => <option key={icon} value={icon}>{icon}</option>)}
        </select>
        ...
        <textarea placeholder="Mô tả dịch vụ" value={serviceForm.description} onChange={(e) => setServiceForm({ ...serviceForm, description: e.target.value })} />
      ...
      <div className="admin-panel">
        ...
        {services.map((s) => (
          <div className="lead-row" key={s.id}>
            <div><strong>{s.title}</strong><small>{s.slug}</small></div>
